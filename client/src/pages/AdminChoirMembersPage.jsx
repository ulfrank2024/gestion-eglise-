...
const AdminChoirMembersPage = () => {
  ...
  const handleAddMember = async () => {
    try {
      await api.admin.addChoirMember(formData);
      setIsAddModalOpen(false);
      setFormData({ member_id: '', voice_type: 'autre', is_lead: false, notes: '' });
      fetchData();
    } catch (err) {
  ...
  const filteredMembers = choirMembers.filter(member => {
    const matchesSearch = member.member?.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const availableMembers = churchMembers.filter(
    member => !choirMembers.some(cm => cm.member_id === member.id)
  );
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">{chorister.member?.email}</p>
                  </div>
                </div>
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-md mx-4 shadow-2xl">
            ...
            <div className="p-4 space-y-4">
              <div className="flex items-center gap-3 p-3 bg-gray-700/50 rounded-lg">
                {selectedMember.member?.profile_photo_url ? (
                  <img
                    src={selectedMember.member.profile_photo_url}
                    alt={selectedMember.member.full_name}
                ...
                <div>
                  <p className="font-medium text-white">{selectedMember.member?.full_name}</p>
                  <p className="text-sm text-gray-400">{selectedMember.member?.email}</p>
                </div>
              </div>
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-md mx-4 shadow-2xl">
            <div className="p-6 text-center">
              ...
              <p className="text-gray-400">
                {t('choir.remove_chorister_confirm', { name: selectedMember.member?.full_name })}
              </p>
            </div>
